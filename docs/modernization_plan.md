...
1. ✅ Модернизация экрана подключения (ScanFragmentNew с карточками устройств)
2. ✅ Переработка экрана деталей (InfoFragmentNew с фокусом на температуры)
3. ✅ Расширение экрана настроек (SettingsFragmentNew с протоколами)
